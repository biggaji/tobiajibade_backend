...
let mail = (0, nodemailer_1.createTransport)({
    service: "gmail",
    auth: {
...
class DBControl {
    ...
    async processHireRequest(full_name, mail, price, place_of_work, time_frame) {
        try {
            let saveHireRequest = await configs_1.default.query(`INSERT INTO hire_request(fullname, email, budget, company, launch_timeframe) 
    ...
    async processContactRequest(full_name, mail, msg) {
        try {
            let saveContactRequest = await configs_1.default.query(`INSERT INTO contact_request(fullname, email, message) 
